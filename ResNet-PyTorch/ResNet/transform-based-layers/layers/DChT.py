...
def chebyshev_transform_matrix(n, device='cpu'):
    """
    Generates the Chebyshev transform matrix directly on the specified device.
    """
    T = torch.zeros((n, n), device=device)
    for k in range(n):
        x = torch.cos((2 * torch.arange(n, device=device) + 1) * torch.pi / (2 * n))
        T[:, k] = torch.from_numpy(eval_chebyt(k, x.cpu().numpy())).to(device)
    return T

def discrete_chebyshev_transform(u, axis=-1):
    if axis != -1:
        u = torch.transpose(u, -1, axis)
    
    n = u.shape[-1]
    T = chebyshev_transform_matrix(n, device=u.device)  # Ensure matrix is on the same device
    y = u @ T
...
def inverse_discrete_chebyshev_transform(u, axis=-1):
    if axis != -1:
        u = torch.transpose(u, -1, axis)
    
    n = u.shape[-1]
    T_inv = torch.linalg.inv(chebyshev_transform_matrix(n, device=u.device)) 
    y = u @ T_inv
...
class DChT1D(torch.nn.Module):
    """
    1D DChT layer. We apply analysis along the last axis. 
    ...
    def forward(self, x):
        ...
        f0 = x
        f1 = discrete_chebyshev_transform(f0)
        if self.retain_DC:
            f2 = self.v * f1[..., 1:]
            f3 = self.ST(f2)
            f4 = inverse_discrete_chebyshev_transform(torch.cat((f1[..., 0:1], f3), axis=-1))
        else:
            f2 = self.v * f1
            f3 = self.ST(f2)
            f4 = inverse_discrete_chebyshev_transform(f3)
        y = f4
        if self.residual:
            y = y + x
        return y

class DChTConv2D(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        f1 = discrete_chebyshev_transform(f0, axis=-1)
        f2 = discrete_chebyshev_transform(f1, axis=-2)
        ...
        f7 = inverse_discrete_chebyshev_transform(f6, axis=-1)
        f8 = inverse_discrete_chebyshev_transform(f7, axis=-2)
...
# Test the forward and inverse transform
x = torch.randn(10, device='cpu')  # Example 1D vector
y = discrete_chebyshev_transform(x)
x_reconstructed = inverse_discrete_chebyshev_transform(y)

print("Original x: ", x)
print("Reconstructed x: ", x_reconstructed)
print("Difference: ", torch.norm(x - x_reconstructed))
